# 30-Seconds-Compatible-with-Micrypython (It doesn't work right now later on 8-04-26 it willl be fixed)-
...
WARNING:
-
Because there is no time module on the fx-CG50, I used busy-wait loops to keep track of the time. These may use alot of battery, but how bad it is is not something I know. This was made for the fx-CG50 so the wait loop probably won't be 30 seconds on other calculators. If you want to change the time so it's 30 seconds on your calculator change n. n = 137500 for ~ 30 seconds on the fx-CG50. You have to experiment with it a little bit so it is right on your non fx-CG50 calculator. Go look at the line of hashtags to find the function.
